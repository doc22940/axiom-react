...
<Story name="DropdownWithState">{stories.DropdownWithState()}</Story>
...
<Preview>
  <Story name="Pick from Menu">{stories.MenuDropdown()}</Story>
</Preview>
...
#### DropdownMenu
You will often want to include `<DropdownMenu>` as part of the  `<DropdownSource>`. With this component you can form include Menu groups build mutliselect Menus.

### DropdownHeader & Footer
...
## Positioning
You can use the *position* and *offset* prop to control where the Source is displayed relative to the Target. PopperJS is used to manage the placement of the Dropdown Source. See [Version 1 of Popper.js](https://popper.js.org/docs/v1/) and [Position](https://github.com/BrandwatchLtd/axiom-react/blob/master/packages/axiom-components/src/Position/Position.js) for more information.
...
To do this you must pass a ref into `<Dropdown>`. You can then call the instance methods on the underlieing component e.g. `dropdownRef.current.close()` or `dropdownRef.current.open()`
